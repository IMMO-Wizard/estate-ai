...
def gen_image(prompt, out_path, reference_image_url=None, original_ref_url=None):
    ...
    - Mit Referenzbild: nano-banana-2-edit (img2img) → maximale Konsistenz
      Parameter: images_list = [ref_url], prompt beschreibt NUR was sich ändert
    - Ohne Referenzbild: bytedance/seedream/v4/text-to-image (Phase 7, Wald)

    Laut Open-Higgsfield Docs: model="nano-banana-2-edit", images_list=[...]
    """
    if reference_image_url:
        # Image-to-Image: Nano Banana 2 Edit
        # Referenzbild als visueller Anker — nur Bauzustand ändert sich
        args = {
            "model": "nano-banana-2-edit",
            "images_list": [reference_image_url],
            "prompt": prompt,
            "resolution": "2K",
            "aspect_ratio": "16:9",
        }
        try:
            # Endpoint laut Open-Higgsfield Docs
            r = higgsfield_client.subscribe(
                "nano-banana/v2/image-to-image",
                arguments=args
            )
            ...
            print(f"img2img Fehler ({e}), fallback auf text2img")

    # Fallback oder Phase 7 (Wald): Text-to-Image
    args = {
        "prompt": prompt,
        "resolution": "2K",
        "aspect_ratio": "16:9",
...
def gen_vid_transition(s, e, prompt, out, dur=10):
    ...
    candidates = [
        # JS SDK zeigt: /v1/image2video/dop mit model-Parameter
        # Kling Start/End Frame Varianten basierend auf echtem SDK-Format
        ("/v1/image2video/kling", {
            "model": "kling-2.5-turbo",
            "prompt": prompt,
            "duration": dur,
            "input_images": [
                {"type": "image_url", "image_url": img_to_data_url(s)},
            ],
            "tail_image": img_to_data_url(e),
        }),
        # Kling 2.1 Master (älteres stabiles Modell mit Start/End)
        ("/v1/image2video/kling", {
            "model": "kling-2.1",
            "prompt": prompt,
            "duration": dur,
            "input_images": [{"type": "image_url", "image_url": img_to_data_url(s)}],
            "tail_image": img_to_data_url(e),
        }),
        # image_tail Format (ComfyUI Doku)
        ("/v1/image2video/kling", {
            "model": "kling-2.5-turbo",
            "prompt": prompt,
            "duration": dur,
        ...
        # DOP Turbo (im JS SDK explizit gezeigt)
        ("/v1/image2video/dop", {
            "model": "dop-turbo",
            "prompt": prompt,
            "input_images": [
                {"type": "image_url", "image_url": img_to_data_url(s)},
                {"type": "image_url", "image_url": img_to_data_url(e)},
            ],
        }),
        # Kling O1 mit neuem Format
        ("/v1/image2video/kling", {
            "model": "kling-o1",
            "prompt": prompt,
            "duration": dur,
