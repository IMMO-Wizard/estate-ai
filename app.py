...
<div class="container">
  ...
  <div class="pwa-banner" id="pwa-banner">
    <img src="/public/icon-192.png" style="width:40px;height:40px;border-radius:10px;" alt="">
  ...
  <div class="header">
    <div class="logo-wrap">
      <img src="/public/icon-192.png" class="logo-img" alt="Immo-Wizard">
      <div class="logo-text">
        <div class="logo-name">
...
def run_pipeline(job_id, img_path, prop_name, prop_type, job_dir):
    ...
    try:
        ...
        for key, fname, tidx, pnum in img_jobs:
            ...
            path = gen_image(prompts["images"][key], img_dir/fname)
            images[key] = str(path)
            if tidx:
        ...
        upd(job_id, progress=64, message="Dolly In...", log_icon="🎥", log_entry="Dolly ran an Handwerker")
        videos["dolly_in"] = str(gen_vid_transition(i["img3"],i["img_craftsman"],p["dolly_in"],vid_dir/"d_in.mp4",5))
        time.sleep(2)
        upd(job_id, progress=67, message="Handwerker aktiv...", log_icon="👷", log_entry="Handwerker — Blick in Kamera")
        videos["dolly_active"] = str(gen_vid_i2v(i["img_craftsman"],p["dolly_active"],vid_dir/"d_act.mp4",7))
        time.sleep(2)
        upd(job_id, progress=70, message="Dolly Out...", log_icon="🎥", log_entry="Zurück zur Totale")
        videos["dolly_out"] = str(gen_vid_transition(i["img_craftsman"],i["img2"],p["dolly_out"],vid_dir/"d_out.mp4",5))
        time.sleep(2)
...
Erstelle JSON für KI-Video-Pipeline (Bau rückwärts als TimeLapse).
Antworte NUR mit JSON:
{{"images":{{"img1":"Aerial Golden Hour fertig 80+ Wörter same high-angle perspective ultra-realistic",
"img2":"Fast fertig Morgenlicht 60+ Wörter same perspective","img3":"Rohbau Betonrahmen bedeckt 60+ Wörter same perspective",
"img4":"Fundament Sonnenuntergang 60+ Wörter same perspective","img5":"Aushub Nachmittag 60+ Wörter same perspective",
"img6":"Planiert sonnig 50+ Wörter same perspective","img7":"Wald natürlich 40+ Wörter",
"img_craftsman":"Maurer Nahaufnahme Rohbau cinematic 16:9 50+ Wörter"}},
"videos":{{"v1":"Wald→Planierung timelapse statisch","v2":"Planierung→Aushub timelapse statisch",
"v3":"Aushub→Fundament timelapse statisch","v4":"Fundament→Rohbau timelapse Kräne statisch",
"v5":"Rohbau→Ausbau timelapse statisch","v6":"Ausbau→Fertig timelapse statisch",
"v7":"Crane down dolly forward smooth golden hour","dolly_in":"Smooth dolly Totale→Maurer kein Cut",
"dolly_active":"Maurer rhythmisch arbeitet blickt kurz in Kamera dokumentarisch",
"dolly_out":"Smooth dolly zurück hoch Maurer wird klein Baustelle sichtbar"}},
"punchlines":["Spruch1 max 8 Wörter Deutsch","Spruch2 max 8 Wörter","Spruch3 max 8 Wörter"]}}"""}
        ]}]
    )
...
def gen_image(prompt, out_path):
    r = higgsfield_client.subscribe("bytedance/seedream/v4/text-to-image",
...
def gen_vid_transition(s, e, prompt, out, dur=10):
    r = higgsfield_client.subscribe("kling/v1/image-to-video",
        arguments={"start_image_url":img_to_data_url(s),"end_image_url":img_to_data_url(e),
                   "prompt":prompt,"duration":dur,"mode":"std","cfg_scale":0.5})
